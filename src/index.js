...
async function createTicket(interaction, category, size) {
  const active = db.prepare(
    "SELECT * FROM tickets WHERE guild_id=? AND user_id=? AND status!='completed'"
...
async function claimTicket(interaction) {
  ...
  db.prepare("UPDATE tickets SET worker_id=? WHERE channel_id=?")
  ...
  return interaction.reply({
    content: `✅ Worker **${t.order_id}** sekarang <@${interaction.user.id}>.`,
    ephemeral: true
  });
}
...
async function closeTicket(interaction) {
  ...
  await interaction.reply({
    content: `✅ ${updated.order_id} selesai. Ticket akan dihapus dalam 60 detik.`,
    ephemeral: true
  });

  setTimeout(() => {
    interaction.channel.delete("DZS Commission completed").catch(() => {});
  }, 60000);
}
  ...
  try {
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId.startsWith("dzs_review_")) {
        ...
        return interaction.reply({
          content: `✅ Feedback **${oid}** berhasil disimpan! 💜`,
          ephemeral: true
        });
      }
    }
